...
const releaseNotesGeneratorOptions = {
  writerOpts: {
    transform: (commit, context) => {
      ...
      // Create a new commit object instead of modifying the immutable one
      const transformedCommit = { ...commit };

      transformedCommit.type = types[commit.type];
      ...
      if (typeof commit.subject === 'string') {
        ...
        if (url) {
          url = `${url}/issues/`;
          // Issue URLs.
          transformedCommit.subject = commit.subject.replace(/#([0-9]+)/g, (_, issue) => {
            issues.push(issue);
